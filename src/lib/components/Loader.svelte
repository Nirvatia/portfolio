...
{#if visible}
	<div
		...
		class="fixed inset-0 z-50 flex items-center justify-center bg-paper
			transition-opacity duration-500 print:hidden"
		class:opacity-0={loader.finished}
		class:pointer-events-none={loader.finished}
	>
		<div class="flex w-[min(340px,82vw)] flex-col gap-6">
			<div class="flex items-center gap-2.5 font-mono text-[11px] uppercase tracking-[0.2em] text-mut">
				<span class="dot-pulse size-[7px] flex-none rounded-full bg-mark"></span>
				идёт запись
			</div>

			<div class="flex min-h-[120px] flex-col gap-[7px] font-mono text-xs leading-normal text-mut">
